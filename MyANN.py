...
class Average_Pool:
    ...
    Realizowana pętlami bez im2col, stosunkowo prosta implementacja.
    """
    def forward(self, M, stride=1, KernShape=2):
        ...
        ---------
        M : np.array
            Dane wejściowe o kształcie (xImg, yImg, numChan, numImds).
        stride : int
            Krok przesuwania okna poolingowego.
        KernShape : int
            Rozmiar kwadratowego okna poolingowego.

        Zwraca
        -------
        self.output : np.array
            Wynik pooling (xOutput, yOutput, numChan, numImds).
        """
        ...
        # Ręczne liczenie Average Pool (pętlami)
            ...
            for y in range(yOutput):
                for x in range(xOutput):
                    for c in range(numChan):
                        ...
                        current_slice = currentIm_pad[x_start:x_end, y_start:y_end, c]
    ...
    def backward(self, dvalues):
        """
        Backward pass dla Average Pooling.

        Parametry
        ---------
        dvalues : np.array
            Gradient błędu względem wyjścia warstwy pooling (xOut, yOut, numChan, numImds).

        Zwraca
        -------
        self.dinputs : np.array
            Gradient błędu względem wejścia (xImg, yImg, numChan, numImds).
        """
        xd, yd, numChan, numImds = dvalues.shape
        
        imagePadded = self.impad
        dinputs = np.zeros(imagePadded.shape)
        Ones = np.ones(imagePadded.shape)  # do rozpraszania gradientu
        
        stride = self.stride
        xK = self.xKernShape
        yK = self.yKernShape
        
        # Każdy piksel w oknie pooling jest ważony przez 1/(xK*yK)
        Ones = Ones / (xK*yK)
        
        # Rozprowadzamy gradient wstecz
        for i in range(numImds):
            for y in range(yd):
                for x in range(xd):
                    for c in range(numChan):
                        ...
                        dinputs[x_start:x_end, y_start:y_end, c, i] += \
                            Ones[x_start:x_end, y_start:y_end, c, i] * dvalues[x, y, c, i]

...
class MyLeNet1:
    ...
    def RunTraining(self,
        ...
        # Zmieniamy kształt monitorowania tak, żeby dodać 2 kolumny na test accuracy i test loss
        Monitor = np.zeros((epochs, 5))
        # Kolejność w Monitor:
        # 0: train_accuracy, 1: train_loss, 2: learning_rate,
        # 3: test_accuracy, 4: test_loss
        ...
        for e in range(epochs):
            ...
            # Średnia train accuracy / train loss z tej epoki
            train_accuracy = epoch_acc_sum / iterations
            train_loss     = epoch_loss_sum / iterations
            Monitor[e, 0]  = train_accuracy
            Monitor[e, 1]  = train_loss
            Monitor[e, 2]  = optimizer.current_learning_rate
    
            # === Nowe: mierzymy test accuracy / test loss po każdej epoce ===
            test_accuracy, test_loss = self.EvaluateTestSet()
            Monitor[e, 3] = test_accuracy
            Monitor[e, 4] = test_loss
    
            print(f"[Epoch {e+1}/{epochs}] train_acc: {train_accuracy:.3f}, train_loss: {train_loss:.3f}, "
                  f"test_acc: {test_accuracy:.3f}, test_loss: {test_loss:.3f}, lr: {optimizer.current_learning_rate:.5f}")
    
            # Zapis wag i monitoringu
...
def PlotMyLeNetResults(Monitor, epochs=10):
    """
    Tworzy wykresy Accuracy i Loss dla train i test w funkcji epok.
    Monitor ma kształt (epochs, 5):
      [train_acc, train_loss, lr, test_acc, test_loss].
    """
    ...
    # 1) Wykres accuracy
    axes[0].plot(x_epochs, Monitor[:epochs, 0]*100, 'o-', color='blue', label='Train Accuracy')
    axes[0].plot(x_epochs, Monitor[:epochs, 3]*100, 'x--', color='green', label='Test Accuracy')
    axes[0].set_xlabel('Epoch')
    axes[0].set_ylabel('Accuracy [%]')
    axes[0].set_title('Accuracy per Epoch')
    axes[0].set_ylim(0, 100)
    # Ustawienie legendy na dole
    axes[0].legend(loc='lower right')

    # 2) Wykres loss
    axes[1].plot(x_epochs, Monitor[:epochs, 1], 'o-', color='red', label='Train Loss')
    axes[1].plot(x_epochs, Monitor[:epochs, 4], 'x--', color='green', label='Test Loss')
    axes[1].set_xlabel('Epoch')
    axes[1].set_ylabel('Loss')
    axes[1].set_title('Loss per Epoch')
    axes[1].legend()

    plt.suptitle('MyLeNet Training vs. Test')
    plt.tight_layout()
    plt.show()
